...
async function isValidSymlink(path) {
    try {
...
export async function status(root) {
    ...
    if (!(await fileExists(configPath))) {
        console.log("");
        console.log(chalk.yellow("  Not initialized. Run `ai init` to set up this repo."));
        console.log("");
        return;
    }
    let config;
    ...
    console.log(`  Context:    ${contextExists ? chalk.green("✓ .ai/context.md") : chalk.red("✗ missing")}`);
    // Learnings
    ...
    if (await fileExists(learningsPath)) {
        ...
        console.log(`  Learnings:  ${chalk.green(`✓ ${entryCount} entries`)}`);
    }
    else {
    ...
    for (const editor of EDITORS) {
        ...
        if (isConfigured) {
            ...
            if (valid) {
                ...
                console.log(`    ${chalk.red("✗")} ${editor.name} ${chalk.dim(`(${editor.path})`)} — ${chalk.red("broken symlink")}`);
            }
        }
        else if (await fileExists(targetPath)) {
            console.log(`    ${chalk.dim("○")} ${editor.name} ${chalk.dim(`(${editor.path} exists but not managed)`)}`);
        }
    }
    console.log("");
}
