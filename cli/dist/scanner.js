...
async function scanStructure(root, maxDepth = 3) {
    const entries = [];
...
function getActiveDirectories(root) {
    try {
        const output = execSync("git log --oneline -50 --name-only --pretty=format: 2>/dev/null | sort | uniq -c | sort -rn | head -20", { cwd: root, encoding: "utf-8", timeout: 5000 });
        const dirs = new Set();
...
export function getCommitsSince(root, sinceCommit) {
    try {
...
export async function scanRepo(root) {
    ...
    const [nodeResult, pythonResult, rubyResult, goResult, rustResult] = await Promise.all([
        scanNode(root),
        scanPython(root),
        scanRuby(root),
        scanGo(root),
    ...
    const results = [nodeResult, pythonResult, rubyResult, goResult, rustResult].filter((r) => r !== null);
