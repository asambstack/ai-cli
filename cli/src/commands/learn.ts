...
import { LEARNING_CATEGORIES, type LearningCategory } from "../types.js";
...
export async function learn(
  ...
  if (!(await fileExists(learningsPath))) {
    console.log(
      chalk.yellow("  No .ai/learnings.md found. Run `ai init` first.")
    );
    return;
  }
  ...
  if (options.category) {
    ...
    if (!valid) {
      console.log(
        chalk.red(`  Unknown category: ${options.category}`)
      );
      console.log(
        `  Valid categories: ${LEARNING_CATEGORIES.map((c) => c.value).join(", ")}`
      );
      return;
    }
  ...
  console.log("");
  console.log(
    `  ${chalk.green("✓")} Added to ${chalk.white(label)}: ${text}`
  );
  console.log("");
}
