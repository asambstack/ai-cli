...
import {
  ...
  LearningEditForm,
  SyncResult,
  WorkspaceKnowledgeList,
  WorkspaceLearningEditForm,
} from "../views/repos.js";
...
app.get("/workspace", async (c) => {
  ...
  return c.html(
    <WorkspacePage
      workspaceLearnings={workspaceLearnings}
...
app.get("/repos", async (c) => {
  const repos = await listRepos(WORKSPACE_ROOT);
  return c.html(<ReposPage repos={repos} />);
});
...
app.get("/repos/:name", async (c) => {
  ...
  return c.html(<RepoDetailPage detail={detail} learnings={learnings} />);
});
  ...
  try {
    ...
    if (c.req.header("HX-Request")) {
      return c.html(
        <SyncResult
          success={true}
          message={`Synced ${name} — context regenerated, symlinks updated.`}
        />
      );
    }
    ...
    if (c.req.header("HX-Request")) {
      return c.html(
        <SyncResult
          success={false}
          message={`Sync failed: ${message.slice(0, 200)}`}
        />
      );
    }
...
app.post("/api/repos/:name/init", async (c) => {
  ...
  if (editorList.length === 0) {
    if (c.req.header("HX-Request")) {
      return c.html(
        <SyncResult success={false} message="No editors selected." />
      );
    }
  ...
  try {
    ...
    if (c.req.header("HX-Request")) {
      return c.html(
        <SyncResult
          success={true}
          message={`Initialized ${name} with editors: ${editorList.join(", ")}`}
        />
      );
    }
    return c.json({ ok: true });
    ...
    if (c.req.header("HX-Request")) {
      return c.html(
        <SyncResult
          success={false}
          message={`Init failed: ${message.slice(0, 200)}`}
        />
      );
    }
    return c.json({ ok: false, error: message }, 500);
...
app.get("/api/workspace/learnings", async (c) => {
  ...
  if (c.req.header("HX-Request")) {
    return c.html(<WorkspaceKnowledgeList learnings={learnings} />);
  }
...
app.get("/api/workspace/learnings/:index/edit", async (c) => {
  ...
  return c.html(<WorkspaceLearningEditForm entry={entry} />);
});

app.post("/api/workspace/learnings", async (c) => {
  const body = await c.req.parseBody();
  const category = body.category as LearningCategory;
  const text = (body.text as string)?.trim();

  if (!text || !category) return c.text("Missing fields", 400);

  const valid = LEARNING_CATEGORIES.find((cat) => cat.value === category);
  if (!valid) return c.text("Invalid category", 400);
...
app.get("/api/repos/:name/learnings", async (c) => {
  ...
  if (c.req.header("HX-Request")) {
    return c.html(<KnowledgeList learnings={learnings} repoName={name} />);
  }
  return c.json(learnings);
});
...
app.get("/api/repos/:name/learnings/:index/edit", async (c) => {
  ...
  if (!entry) return c.text("Not found", 404);
  return c.html(<LearningEditForm entry={entry} repoName={name} />);
});
