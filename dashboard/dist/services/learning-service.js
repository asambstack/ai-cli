...
function getLearningsPath(root) {
...
export async function getLearnings(root) {
...
export async function addLearning(root, category, text) {
    const path = getLearningsPath(root);
    if (!(await fileExists(path))) {
...
export async function updateLearning(root, targetIndex, newText) {
    const path = getLearningsPath(root);
    const content = await readFile(path, "utf-8");
    const lines = content.split("\n");
    let entryIndex = 0;
...
export async function deleteLearning(root, targetIndex) {
    const path = getLearningsPath(root);
    const content = await readFile(path, "utf-8");
    const lines = content.split("\n");
    let entryIndex = 0;
