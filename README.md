...
All user-specific paths are in `devenv/config.yaml` (gitignored). First-time setup:

```bash
# New machine — install prereqs + generate config
bash ~/.ai-agents-repo/devenv/bootstrap.sh

# Or just generate config (if prereqs already installed)
...
Files:
- `devenv/config.yaml.sample` — template with documented defaults (committed)
- `devenv/config.yaml` — your local config (gitignored, generated by `setup-config.sh`)
- `devenv/docker-compose.generated.yml` — auto-generated from config (gitignored)
